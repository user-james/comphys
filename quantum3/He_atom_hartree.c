...
int main( )
{
    /*  This program finds the energy eigenvalues and wave functions of the electrons in
        ...
        [W. Kohn & L. J. Sham, Physical Review 140, A1133ÐA1138 (1965). 
         See also http://en.wikipedia.org/wiki/KohnÐSham_equations]
    ...
    */
    ...
    energy = fopen("./my_files/ex2/energies", "w");
    for( it=0; it<nt; it++ )               /* iterate the self-consistent field equations nt times */
    {
        ...
        for( i=1; i< nr ; i++ )
        {
            Q[i] = Q[i-1]+ 2*psi[0][i]*psi[0][i]*rab[i];      /* charge density for double-occupancy of lowest energy orbital  */
        }
        ...
        sprintf(rho_file, "./my_files/ex2/charge_density_iter_%d", it);
