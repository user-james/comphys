...
/* functions used    */   
     ...
     int dynmat(double *, double *, double *, int, double *, double *, double *, double,
                double, double, double, double, double rij[][NMAX], double *);
...
/*     -----------------------------------------------------------------------------
     ...
     for(iscale = -nscale; iscale < nscale+1; iscale++)
     {
         ...
         dynmat( xs,ys,zs, N, vx,vy,vz, dx, alpha,beta,L,V0, rij, D);      /*  find the dynamical matrix, diagonalize it and return its eigenvalues  */
    ...
    // prints eigenvalues to e_vals.txt 
    evals = fopen("e_values.txt", "w");
    for(i=0; i<20; i++){
...
int dynmat(double x[], double y[], double z[], int N, double vx[], double vy[], double vz[], double dx,
           double alpha, double beta, double L, double V0, double rij[][NMAX], double D[])
{
    /*   -----------------------------------------------------------------------------
    ...
    // printing e-vectors to file 
    evects = fopen("e_vects.txt", "w");
    for(j=0; j<10; j++){    
        for(i=0; i<3*N; i++){
            fprintf(evects, "%lf+i%lf\n", ZR[i][j], ZI[i][j]);
        }
        fprintf(evects, "\n\n");
    }
    fclose(evects);  
