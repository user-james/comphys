...
/*************************************************************************/
int bin_rij_PBC_quantum(double rij[][NMAX][4], int N, int key)
{
    /* this function accumulates statistics of the atomic pair-correlation function 
     ...
     INPUT:
         rij = current interatomic distances [3rd index = 0] and vectors [3rd index = 1,2,3]
    ...
    switch (key)    
    {
        ...
        case 2:                   /* if key = 2, output histogram to file  */
            ...
            sprintf(filename, "./data/pair_distribution_%d", i_file);
            i_file += 1;
            outhist = fopen(filename, "w");
        
            for(i_hist=0; i_hist<NHIST; i_hist++)
            {
                fprintf(outhist, "%lf\t%lf\n", (i_hist*RMAX)/NHIST, (double)rij_hist[i_hist]/(double)n_sample/N*2/RMAX*NHIST/4/3.1415926/(i_hist*RMAX)*NHIST/(i_hist*RMAX)*NHIST );
            }
